...
class Player(object):
    ...
    async def get(self):
        ...
        query = """SELECT users.user_id, users.steam_id, users.discord_id, users.name, users.pfp, users.joined, IFNULL(statistics.total_time, 0) AS total_time, IFNULL(statistics.elo, 0) AS elo, IFNULL(statistics.kills, 0) AS kills, 
                    ...
                    WHERE (users.steam_id = :user_id OR users.discord_id = :user_id OR users.user_id = :user_id) AND statistics.region = :region AND statistics.league_id = :league_id
                """
    ...
    async def reset(self):
        ...
        await self.current_league.database.execute(query=query, values=self.values)
